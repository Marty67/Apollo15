...
func _physics_process(delta):
	...
	if $maploader.map.has_node("boss"):
		if Global.Boss.getHealth() <=0:
			Global.Ending.play()
		
	
	
	if $maploader.has_node("player"):
		if Global.Player.get_components() == 4:
			
			$maploader/Interface/complete.show()
			Global.Player.reset_components()
			Global.Player.hasComponents()
			timer.start()
...
func showTask():
	$maploader/Interface/start.show()
	
func hideTask():
	$maploader/Interface/start.hide()
